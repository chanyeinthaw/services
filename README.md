...
| PostgreSQL 18.4 | `pg.database.internal` | `developer` | `dev-postgres` | `development` |
...
psql -h pg.database.internal -U developer -d development
...
MinIO stores development objects under `apps/minio/data/`. The S3-compatible API is available directly on localhost at `http://127.0.0.1:9000`; the browser console is proxied by Caddy at `https://minio.app.internal` from `127.0.0.1:9001`.
...
For SDKs running directly on this host, use endpoint `http://127.0.0.1:9000` and path-style addressing if the SDK requires it. These credentials and this deployment are for development only.
